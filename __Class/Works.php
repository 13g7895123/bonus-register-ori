...
class SYSAction
{
    ...
    //     curl_close($ch);
    // }
    // public static function MIKI_sms_send($c, $p, $user = MIKI_SMS_USERNAME, $pwd = MIKI_SMS_PASSWORD)
    // {
    //     // echo 'http://api.message.net.tw/send.php?id=' . $user . '&password=' . $pwd . '&tel=' . $p . '&msg=' . urlencode($c) . '&mtype=G&encoding=utf8';
    //     $ch = curl_init();
    //     curl_setopt($ch, CURLOPT_URL, 'http://api.message.net.tw/send.php?id=' . $user . '&password=' . $pwd . '&tel=' . $p . '&msg=' . urlencode($c) . '&mtype=G&encoding=utf8');
    //     curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); //將curl_exec()獲取的訊息以文件流的形式返回，而不是直接輸出。 這參數很重要 因為如果有輸出的話你api 解析json時會有錯誤
    //     curl_exec($ch);
...
class Line
{
    /**
     * 透過 Line BOT 發送訊息
     * @param array $userIds 欲傳送之 Line User ID 之陣列
     * @param string $message 欲傳送之訊息
     * @param string $channelAccessToken Line API 的 Token KEY
     */
    public static function push($userIds = [], $message, $channelAccessToken)
    {
